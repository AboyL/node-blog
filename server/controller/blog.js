...
const getBlogDetail = async (id) => {
  const sql = `select * from ${TABLE_NAME} where id=${resolveParam(id)} `
  const result = await exec(sql)
  return result
}
...
const createBlog = async (blogData = {}) => {
  ...
  const sql = `insert into ${TABLE_NAME}(title,author,content,createTime) 
...
const updateBlog = async (id = '', blogData = {}) => {
  if (id) {
    const { title, content, author } = blogData
    const createTime = Date.now()
    const sql = `update ${TABLE_NAME} set title=${resolveParam(title)},content=${resolveParam(content)},createTime=${resolveParam(createTime)} where id=${resolveParam(id)} and author=${resolveParam(author)}`
    const result = await exec(sql)
...
const deleteBlog = async (id, author) => {
  if (id) {
    const sql = `delete from ${TABLE_NAME} where id=${resolveParam(id)} and author=${resolveParam(author)}`
    const result = await exec(sql)
    if (result.affectedRows > 0) {
      return {
        status: true
